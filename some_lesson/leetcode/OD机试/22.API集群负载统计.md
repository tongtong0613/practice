**题目**
```
某个产品的 restful API 集合部署在 服务器 集群的多个节点上，近期对客户端访问日志进行了采集，需要统计各个API的访问频次，根据热点信息
节点之间做负载均衡，现在需要实现热点信息统计查询功能。
    ...
    def solve(self, n, urls, spec):
        ...
        for url in urls:
            ...
            for i in range(len(tmp)):
                if i not in mp:
                    mp[i] = {}
                if tmp[i] not in mp[i]:
                    mp[i][tmp[i]] = 1
                else:
                    mp[i][tmp[i]] += 1
        return mp[spec[0]-1][spec[1]] if spec[1] in mp[spec[0]-1] else 0



s = Solution()
print(s.solve(5, ['/huawei/computing/no/one',
                  '/huawei/computing',
                  '/huawei',
                  '/huawei/cloud/no/one',
                  '/huawei/wireless/no/one'],
